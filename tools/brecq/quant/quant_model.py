...
from tools.brecq.quant.quant_block import specials, BaseQuantBlock
from tools.brecq.quant.quant_layer import QuantModule, StraightThrough, PreQuantModule
from tools.brecq.quant.fold_bn import search_fold_and_remove_bn
...
class QuantModel(nn.Module):
    ...
    def set_quant_state(self, weight_quant: bool = False, act_quant: bool = False):
        for m in self.model.modules():
            if isinstance(m, (QuantModule, BaseQuantBlock, PreQuantModule)):
                m.set_quant_state(weight_quant, act_quant)

    # def forward(self, input):
    #     return self.model(input)

    def forward(self, return_loss, **data):
        return self.model(return_loss=False, **data)

    def extract_feat(self, img):
        return self.model.extract_feat(img)

    def extract_feat_for_ptq(self, img_img_metas_dict, **kwargs):
        return self.model.extract_feat_for_ptq(img_img_metas_dict, **kwargs)
    ...
    def set_first_last_layer_to_8bit(self):
        ...
        print('please set in custom manner')

    def disable_network_output_quantization(self):
        print('Please disable output in a custom manner')
        #module_list = [] 
        #for m in self.model.modules():
        #    if isinstance(m, QuantModule):
        #        module_list += [m]
