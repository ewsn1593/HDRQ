...
class UniformAffineQuantizer(nn.Module):
    ...
    """
    def __init__(self, n_bits: int = 8, symmetric: bool = True, channel_wise: bool = False, scale_method: str = 'max',
        ...
        if symmetric:
            ...
            print('Symmetric Restricted Quantization')
        else:
            self.n_levels = 2 ** (self.n_bits)
            print('Restricted Quantization')
        self.delta = None
    ...
    def forward(self, x: torch.Tensor):
        ...
        x_dequant = (x_quant) * self.delta #(x_quant - self.zero_point) * self.delta
        return x_dequant
...
class PreQuantModule(nn.Module):
    """
    Quantize before module
    """
    def __init__(self, org_module: Union[nn.Linear], weight_quant_params: dict = {},
                 act_quant_params: dict = {}, disable_act_quant: bool = False, se_module=None, sym=None):
        super(PreQuantModule, self).__init__()
        if isinstance(org_module, nn.Conv2d):
            self.fwd_kwargs = dict(stride=org_module.stride, padding=org_module.padding,
                                   dilation=org_module.dilation, groups=org_module.groups)
            self.fwd_func = F.conv2d
        ...
        if sym is not None:
            ...
            self.act_quantizer = UniformAffineQuantizer(**sym_act_quant_params)
        else:
            self.act_quantizer = UniformAffineQuantizer(**act_quant_params)
    ...
    def forward(self, input: torch.Tensor):
        ...
        if not self.always_xactquant:
            if self.use_act_quant:
                out = self.act_quantizer(input)
            else:
                out = input
        else: out = input

        # Fwd
        out = self.fwd_func(out, weight, bias, **self.fwd_kwargs)
