...
from tools.brecq.quant.quant_layer import QuantModule, StraightThrough, PreQuantModule, lp_loss
from tools.brecq.quant.quant_model import QuantModel
from tools.brecq.quant.quant_block import BaseQuantBlock
from tools.brecq.quant.adaptive_rounding import AdaRoundQuantizer
from tools.brecq.quant.data_utils import save_grad_data, save_inp_oup_data
...
def block_reconstruction(model: QuantModel, block: BaseQuantBlock, cali_data: torch.Tensor,
    ...
    if not act_quant:
        ...
        if hasattr(block.act_quantizer, 'delta'):
            opt_params = []
            if block.act_quantizer.delta is not None:
        ...
        if len(opt_params) == 1:
            if opt_params[0] is None:
                print('This quantizer is not used parameter-skip reconstruction')
                return
        if len(opt_params) == 0:
            print('This quantizer is not used parameter-skip reconstruction')
            return
    ...
    for i in range(iters):
        ...
        if isinstance(model.module.model.backbone, MixVisionTransformer):
            if len(cached_inps[idx]) == 3:
                cur_inp, H, W = cached_inps[idx]
                cur_inp = cur_inp.to(device)
                cur_out = cached_outs[idx][0].to(device)
                cur_grad = None
                
                optimizer.zero_grad()
                out_quant = block(cur_inp, H, W)
            elif len(cached_inps[idx]) == 1:
                cur_inp = cached_inps[idx][0].to(device)
                cur_out = cached_outs[idx][0].to(device)
                cur_grad = None
            ...
            if len(out_quant) == 3:
                err = loss_func(out_quant[0], cur_out, cur_grad)
            elif len(out_quant) == 1:
