...
from .fake_quant import AdaRoundFakeQuantize, FixedFakeQuantize, LSQFakeQuantize, LSQPlusFakeQuantize, LSQNoiseFakeQuantize, HDRQFakeQuantize
...
FakeQuantizeDict = {
    ...
    'LSQNoiseFakeQuantize':  LSQNoiseFakeQuantize,
    'HDRQFakeQuantize':      HDRQFakeQuantize,
}
...
def Quantizer(module, config):
    ...
    if module_type in module_type_to_quant_weight:
        ...
        print(f'Not in converting configure - {str(module_type)}')
    return module
...
class PreQuantizedLayer(QuantizedModule):
    def __init__(self, module, activation, w_qconfig, a_qconfig, qoutput=True, sym=None):
        super().__init__()
        self.qoutput = qoutput # means prequant here
        self.module = Quantizer(module, w_qconfig)
        self.activation = activation
        if qoutput:
            if sym is not None:
                sym_act_quant_params = copy.deepcopy(a_qconfig)
                sym_act_quant_params['symmetric'] = sym
                self.layer_post_act_fake_quantize = Quantizer(None, sym_act_quant_params) ###NOTE actually pre layer quantize
            else:
                self.layer_post_act_fake_quantize = Quantizer(None, a_qconfig) ###NOTE actually pre layer quantize
            #self.layer_post_act_fake_quantize = Quantizer(None, a_qconfig)
        self.always_xactquant  = False

    def forward(self, x):
        if not self.always_xactquant:
            if self.qoutput:
                x = self.layer_post_act_fake_quantize(x)
        x = self.module(x)
        if self.activation is not None:
