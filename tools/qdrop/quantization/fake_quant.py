...
from .util_quant import (
    ...
    fake_quantize_learnable_per_tensor_training,
    ...
    fake_noise_quantize_learnable_per_tensor_training, # noise
    fake_noise_quantize_per_tensor, # noise
        ...
        if self.fake_quant_enabled == 1:
            ...
            if self.ch_axis != -1:
                ...
                if self.use_grad_scaling:
                    ...
                    grad_factor = 1.0
                X = fake_quantize_learnable_per_tensor_training( # without zeropoint
...
class LSQNoiseFakeQuantize(QuantizeBase):

    def __init__(self, observer, bit=8, symmetric=False, ch_axis=-1, use_grad_scaling=True, noise=False):
        super().__init__(observer, bit=bit, symmetric=symmetric, ch_axis=ch_axis)
        self.scale = torch.nn.Parameter(torch.tensor([1.0], dtype=torch.float))
        self.register_buffer('zero_point', torch.tensor([0], dtype=torch.int))
        self.register_buffer('eps', torch.tensor([torch.finfo(torch.float32).eps]))
        ...
        if self.fake_quant_enabled == 1:
            ...
            if self.ch_axis != -1:
                ...
                if self.use_grad_scaling:
                    ...
                    grad_factor = 1.0
                if self.noise:
                    X = fake_noise_quantize_learnable_per_tensor_training( # without zeropoint
                        X, self.scale, self.zero_point.item(), self.quant_min, self.quant_max, grad_factor)
                else:
                    X = fake_quantize_learnable_per_tensor_training( # without zeropoint
                        X, self.scale, self.zero_point.item(), self.quant_min, self.quant_max, grad_factor)
...
class HDRQFakeQuantize(QuantizeBase):
    """
    HDRQ
    """

    def __init__(self, observer, bit=8, symmetric=False, ch_axis=-1, noise=False):
        super().__init__(observer, bit=bit, symmetric=symmetric, ch_axis=ch_axis)
        self.scale = torch.nn.Parameter(torch.tensor([1.0], dtype=torch.float))
        #self.register_buffer('scale', torch.tensor([1.0], dtype=torch.float))
        self.register_buffer('zero_point', torch.tensor([0], dtype=torch.int))
    ...
    def get_hard_value(self, X):
        print('HDRQ - Skip weight modification')
        return X
        ...
        if self.fake_quant_enabled == 1:
            ...
            if not self.adaround:
                if self.ch_axis != -1:
                    ...
                    X = fake_noise_quantize_per_tensor(
                        X, self.scale.item(), self.zero_point.item(),
                        self.quant_min, self.quant_max)
            else:
                if not hasattr(self, 'alpha'):
