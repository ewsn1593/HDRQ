...
# Begin PTQ(qdrop) code (for counting reconstruction)
recon_total_count = 0
...
def main():
    ...
    if isinstance(model.backbone, ResNetV1c):
        ...
        print('ResNet101-Nothing to do for stem layer in QDROP')
    #### for MiT-B5 backbone ####
        ...
        def recon_model(module: nn.Module, fp_module: nn.Module):
            ...
            for name, child_module in module.named_children():
                if isinstance(child_module, (QuantizedLayer, QuantizedBlock, PreQuantizedLayer)):
                    ...
                    reconstruction(model.module, fp_model.module, child_module, getattr(fp_module, name), cali_data, qdrop_config.quant.recon)
                else:
    ...
    try:
        ...
        if isinstance(model.module.backbone, ResNetV1c):
            ...
            name_ = 'default'
        print(f'Save to ./ckpt/{save_name_ds}/QDROP_PTQTest{name_}_W{args.n_bits_w}A{args.n_bits_a}_fixed_{save_name_ds}_seed{args.seed}.pt')
        torch.save(model, f'./ckpt/{save_name_ds}/QDROP_PTQTest{name_}_W{args.n_bits_w}A{args.n_bits_a}_fixed_{save_name_ds}_seed{args.seed}.pt')
    except:
        breakpoint()
