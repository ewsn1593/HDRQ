    ...
    for i, data in enumerate(data_loader):
        ...
        if show or out_dir:
            ...
            for img, img_meta in zip(imgs, img_metas):
                ...
                if hasattr(model.module.model.decode_head,
                           'debug_output_attention') and \
                        model.module.model.decode_head.debug_output_attention:
                    # Attention debug output
                    mmcv.imwrite(result[0] * 255, out_file)
                else:
                    model.module.model.show_result(
                        img_show,
                        result,
                        palette=dataset.PALETTE,
                        show=show,
...
def single_gpu_ptq_test_multi_process(model,
                    data_loader,
                    show=False,
                    out_dir=None,
                    efficient_test=False,
