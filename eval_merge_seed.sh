...
echo "model path1: " $MODEL_PATH1 ", model path2: " $MODEL_PATH2 ", qmethod: " $QMETHOD ", world size (n_gpus): " $WORLD_SIZE ", port num: " $PORT_NUM "SEED" $SEED
python -m evals.eval_merge ${CONFIG_FILE1} ${CONFIG_FILE2} --eval mIoU --show-dir ${SHOW_DIR} --opacity 1 --model_path1 $MODEL_PATH1 --model_path2 $MODEL_PATH2 --world_size $WORLD_SIZE --qmethod $QMETHOD --port_num $PORT_NUM --seed $SEED
