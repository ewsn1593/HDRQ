...
def run_demo(demo_fn, args):
    ...
    # dataset.img_infos=dataset.img_infos[0:24]
    data_loader = None
    ...
    if args.format_only:
        eval_result = dataset.format_results(results, **kwargs)
    if args.eval:
        eval_result = dataset.evaluate(results, args.eval, **kwargs)

    save_txt_str = (args.model_path).split('/')[-1]
    f1 = open('./result_txts/results.txt', 'a')

    f1.write("{0}\n".format(save_txt_str))

    for key in eval_result:
        data = eval_result[key]
        f1.write("{0} ".format(data))
