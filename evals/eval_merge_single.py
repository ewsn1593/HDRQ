...
def parse_args():
    ...
    parser.add_argument('--seed', required=True, type=int, help='random seed for results reproduction')
    ...
    # QDROP BRECQ choice
    parser.add_argument('--qmethod', choices=['BRECQ', 'QDROP', 'HDRQ', 'FP'], type=str, help='Merge models quantized with which scheme?')
...
def demo_basic_single(rank, world_size, args, cfg1, cfg2, dataset1, dataset2, result_queue, cur_iter):
...
def run_demo(demo_fn, args):
    ...
    # dataset1.img_infos=dataset1.img_infos[0:5]
    # dataset2.img_infos=dataset2.img_infos[0:5]
    ...
    for cur_iter in range(5):
        ...
        for i in range(world_size):
            result_queue[0].append([])
            result_queue[1].append([])
            
        result_queue = demo_fn(0, world_size, args, cfg1, cfg2, dataset1, dataset2, result_queue, cur_iter)
            
        results_list1 = result_queue[0]
        results1 = results_list1[0]
        ...
        results_list2 = result_queue[1]
        results2 = results_list2[0]
...
if __name__ == "__main__":
    ...
    beginTime = time.time()
    run_demo(demo_basic_single, args)
