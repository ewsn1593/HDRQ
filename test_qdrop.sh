...
echo ./log/${W_BIT}${A_BIT}test_qdrop_${DATASET_NAME}_seed${SEED}_${NOTE}.txt

python -m tools.test_qdrop ${CONFIG_FILE} ${CHECKPOINT_FILE} --qdrop_config ${QDROP_DIR} --eval mIoU --show-dir ${SHOW_DIR} --opacity 1 --n_bits_w ${W_BIT} --n_bits_a ${A_BIT} --save_ds ${DATASET_NAME} --seed ${SEED} 2>&1 | tee ./log/${W_BIT}${A_BIT}test_qdrop_${DATASET_NAME}_seed${SEED}_${NOTE}.txt
